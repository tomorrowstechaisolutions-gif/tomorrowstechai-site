...
export default async function ServicesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <>
    <div className="cc-greet"><div><h1>Services</h1><p>Manage pricing, profitability, availability, clients, and operational workflows for every service Tomorrow’s Tech AI sells.</p></div><div className="sv-actions"><Link href="/admin/catalog" className="cc-btn">View Catalog</Link>{canManage && <ServiceModal />}</div></div>
    <div className="sv-kpis">{[['Active Services', k.active_services, 'Available operational services'], ['Monthly Recurring Revenue', money(k.mrr_cents), 'Active services · tracked assignments'], ['One-Time Revenue', money(k.one_time_cents), `${period === 'all' ? 'All time' : `This ${period}`} · collected USD`], ['Active Subscriptions', k.subscriptions, 'Active recurring client services'], ...(canManage ? [['Average Margin', marginLabel(k.average_margin), 'Active services with valid costs']] : [])].map(([label, value, note]) => <section className="sv-kpi" key={String(label)}><span>{label}</span><strong>{value}</strong><small>{note}</small></section>)}</div>
    <form className="sv-toolbar"><label>Search<input className="cc-input" name="q" placeholder="Search services..." defaultValue={pick('q')} /></label>
      ...
      <label>Sort<select name="sort" className="cc-select" defaultValue={pick('sort') || 'name'}>{Object.entries({ name:'Name', revenue:'Revenue', clients:'Clients', price:'Price', ...(canManage ? { margin:'Margin' } : {}), updated:'Recently Updated' }).map(([v,l]) => <option key={v} value={v}>{l}</option>)}</select></label>
