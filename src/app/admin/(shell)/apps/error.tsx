...
/**
 * The Apps error boundary.
 *
 * Without this, any failure in a server component under /admin/apps becomes
 * Next's blank "This page couldn't load" — which is true, useless, and sent
 * somebody digging through Vercel logs to discover the migration had not
 * been run. A missing table is the single most likely failure on a screen
 * this new, so the boundary names it and says where to look.
 *
 * It reports the message and Next's digest, because that digest is how a
 * production error is found in the logs.
 */
      ...
      <div className="cc-board">
        <section className="cc-panel cc-s12">
          <div className="cc-panel-head">
            <h2>{missingTable ? "The Apps tables are not in the database yet" : "Apps could not be loaded"}</h2>
          </div>
            ...
            <p className="cc-note" style={{ marginTop: 0 }}>
              {missingTable ? (
                <>
                  ...
                  <code>supabase/migrations/20260910120000_apps.sql</code> from the
