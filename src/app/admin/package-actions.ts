...
function refresh(id?:string){ ["/admin/packages","/admin/services","/admin/catalog","/services","/website-intake","/get-started","/services/ai-business-operator","/services/ai-ad-creative-studio","/services/grow-your-audience","/services/run-your-business"].forEach(path=>revalidatePath(path)); if(id) revalidatePath(`/admin/packages/${id}`); }
...
export async function saveCatalogPackage(_state:ActionResult,fd:FormData):Promise<ActionResult>{
  try {
    ...
    if(file instanceof File && file.size){ const allowedTypes=new Map([["image/png","png"],["image/webp","webp"],["image/jpeg","jpg"]]); const ext=allowedTypes.get(file.type); if(file.size>6*1024*1024||!ext) throw new Error("Choose a PNG, WebP, or JPEG package badge up to 6 MB."); const path=`catalog/${saved}/${crypto.randomUUID()}.${ext}`; const storage=supabaseAdmin().storage.from("brand-assets"); const uploaded=await storage.upload(path,file,{contentType:file.type,upsert:false}); if(uploaded.error) throw new Error("The package saved, but its image upload failed."); const current=await db.from("catalog_items").select("image_path").eq("id",saved).single(); await db.from("catalog_items").update({image_path:path,image_url:null}).eq("id",saved); if(current.data?.image_path) await storage.remove([current.data.image_path]); }
