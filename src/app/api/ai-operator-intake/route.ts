...
export async function POST(req: Request) {
  try {
    ...
    const packagePlan=await getPublicPackage(clean(body.plan,120),"ai");
    const fallbackPlan=getAiOperatorPlan(clean(body.plan,20));
    const plan=packagePlan?{id:packagePlan.slug,name:packagePlan.name,price:catalogPrice(packagePlan),setup:packagePlan.setupFeeCents?`$${(packagePlan.setupFeeCents/100).toLocaleString("en-US")} setup`:"No setup fee",category:packagePlan.category,recordId:packagePlan.id}:{...fallbackPlan,category:"ai",recordId:null};
    ...
    const details = [
      `AI Business Operator intake`, `Service category: ${plan.category}`,`Selected plan: ${plan.name} (${plan.price}; ${plan.setup})`,`Source page: /get-started`,
    ...
    if (result.leadId && supabaseConfigured()) {
      ...
      await db.from("lead_events").insert({ lead_id: result.leadId, type: "operator_intake", body: `${plan.name} intake submitted. Preferred contact: ${contactMethod}.`, actor: "system", meta: { package_id:plan.recordId,plan: plan.id,package_name:plan.name,category:plan.category,price:plan.price,source_page:"/get-started", industry, employees: clean(body.employees, 50), monthly_leads: clean(body.monthlyLeads, 50), challenges: selectedChallenges, preferred_contact: contactMethod, best_time: clean(body.bestTime, 50) } });
    }
