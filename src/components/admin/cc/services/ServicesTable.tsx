...
export default function ServicesTable({ services, canManage }: { services: Service[]; canManage: boolean }) {
  ...
  return <div className="ad-table-scroll sv-table-wrap"><table className="ad-table sv-table"><thead><tr>{['Service', 'Category', 'Billing', 'Price', ...(canManage ? ['Internal Cost', 'Gross Profit', 'Margin'] : []), 'Clients', 'Revenue', ...(canManage ? ['Health'] : []), 'Status', 'Actions'].map(h => <th key={h}>{h}</th>)}</tr></thead>
