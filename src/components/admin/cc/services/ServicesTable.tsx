'use client';
/* eslint-disable @next/next/no-img-element -- authenticated catalog thumbnails use a protected route */
...
export default function ServicesTable({ services, canManage, media }: { services: Service[]; canManage: boolean; media: Record<string,{imagePath:string|null;imageUrl:string|null}> }) {
  ...
  return <div className="ad-table-scroll sv-table-wrap"><table className="ad-table sv-table"><thead><tr>{['Service', 'Category', 'Billing', 'Price', ...(canManage ? ['Internal Cost', 'Gross Profit', 'Margin'] : []), 'Packages', 'Clients', 'Revenue', ...(canManage ? ['Health'] : []), 'Status', 'Actions'].map(h => <th key={h}>{h}</th>)}</tr></thead>
    <tbody>{services.map(s => <tr key={s.id} onClick={e => { if (!(e.target as HTMLElement).closest('a,button,form,details,dialog,input,select,textarea')) router.push(`/admin/services/${s.id}`); }}>
      <td data-label="Service"><div className="sv-service-cell">{media[s.id]?.imagePath||media[s.id]?.imageUrl?<img src={media[s.id].imagePath?`/api/admin/catalog-media/${s.id}`:media[s.id].imageUrl!} alt=""/>:<span className="sv-missing-image">Missing image</span>}<div><Link href={`/admin/services/${s.id}`} className="cc-link sv-service-name"><strong>{s.name}</strong></Link><small>{s.sku || s.service_type}</small></div></div></td>
      ...
      <td data-label="Actions"><details className="sv-menu"><summary aria-label={`Actions for ${s.name}`}>•••</summary><div><Link href={`/admin/services/${s.id}`} className="cc-btn">View</Link>{canManage && <><Link href={`/admin/marketing/ads?catalog=${s.id}`} className="cc-btn">Generate Ad</Link><ServiceModal service={s} /><ServiceModal service={s} duplicate /><StatusAction service={s} status={s.status === 'active' ? 'paused' : 'active'} />{s.status !== 'retired' && <StatusAction service={s} status="retired" />}</>}</div></details></td>
    </tr>)}</tbody></table></div>;
}
